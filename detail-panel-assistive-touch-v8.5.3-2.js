...
(function(){
  ...
  const VERSION = 'v8.5.3-2';
  const STORAGE_KEY = 'dp_assistive_touch__v8.5.3-2';
  ...
  function ensureMinimalStyles(){
    ...
    st.textContent = `
      ...
      detailPanel .dp-at-settings-sheet{ position:fixed; z-index:2400; left:12px; right:12px; bottom:calc(12px + env(safe-area-inset-bottom)); max-height:70vh; overflow:auto; background:rgba(255,255,255,0.92); backdrop-filter:blur(14px) saturate(1.15); -webkit-backdrop-filter:blur(14px) saturate(1.15); border:1px solid rgba(2,6,23,0.14); border-radius:16px; box-shadow:0 18px 50px rgba(2,6,23,0.18); padding:10px; display:none; }
      detailPanel .dp-at-settings-title{ font-weight:950; font-size:13px; margin:2px 2px 10px; color:rgba(15,23,42,0.90); }
      detailPanel .dp-at-row{ display:flex; align-items:center; justify-content:space-between; gap:10px; padding:8px 8px; border-radius:12px; border:1px solid rgba(2,6,23,0.10); background:rgba(255,255,255,0.88); margin-bottom:8px; }
      detailPanel .dp-at-row label{ font-weight:850; font-size:12px; color:rgba(15,23,42,0.86); }
      detailPanel .dp-at-row .hint{ font-size:11px; color:rgba(15,23,42,0.65); font-weight:650; }
      detailPanel .dp-at-actions-grid{ display:grid; grid-template-columns:1fr 1fr; gap:8px; }
      detailPanel .dp-at-chip{ display:flex; align-items:center; justify-content:space-between; gap:10px; padding:10px 10px; border-radius:14px; border:1px solid rgba(2,6,23,0.10); background:rgba(255,255,255,0.90); }
      detailPanel .dp-at-chip .left{ display:flex; align-items:center; gap:10px; min-width:0; }
      detailPanel .dp-at-chip i{ width:18px; text-align:center; color:rgba(15,23,42,0.75); }
      detailPanel .dp-at-chip .name{ font-weight:900; font-size:12px; color:rgba(15,23,42,0.92); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
      detailPanel .dp-at-btns{ display:flex; gap:8px; margin-top:10px; }
      detailPanel .dp-at-btn{ flex:1 1 auto; height:42px; border-radius:14px; border:1px solid rgba(2,6,23,0.12); background:rgba(255,255,255,0.90); font-weight:950; cursor:pointer; }
      detailPanel .dp-at-btn.primary{ background:rgba(102,126,234,0.92); border-color:rgba(102,126,234,0.30); color:#fff; }

      /* JP|VI label block (flex column for text, but wrapped in flex row with icon) */
      detailPanel .dp-label-group { display:flex; flex-direction:column; align-items:flex-start; line-height:1.2; }
      detailPanel .dp-action-btn .dp-label-jp{ font-weight:800; font-size:11.5px; color:rgba(15,23,42,0.95); margin-bottom: 2px; }
      detailPanel .dp-action-btn .dp-label-vi{ font-weight:700; font-size:10px; opacity:1; color:#64748b; }

      /* Settings action look (fallback) */
      detailPanel .dp-action-btn.dp-at-settings-action{ border-color:rgba(2,6,23,0.16); background:rgba(255,255,255,0.96); }

      /* Assistive menu position will be set by JS */
      detailPanel .dp-assistive-menu{ left:auto; right:auto; top:auto; bottom:auto; }
      
      /* ====== MOBILE ACTION BAR (Navibar fix) ====== */
      detailPanel .dp-mobile-actionbar { display:none; }
      @media (max-width: 768px) {
        /* Chỉ show bar khi Panel open, ở tab info, VÀ TRẠNG THÁI JS allow (class .dp-mode-bar) */
        detailPanel.detail-panel.open.dp-mode-bar:has(.detail-tab.active[data-tab="info"]) .dp-mobile-actionbar { display:flex; }
        detailPanel.detail-panel.open .dp-mobile-actionbar { position:fixed; left:0; right:0; bottom:0; z-index:2200; padding:8px 10px calc(8px + env(safe-area-inset-bottom)); background:rgba(255,255,255,0.92); backdrop-filter:blur(16px) saturate(1.2); -webkit-backdrop-filter:blur(16px) saturate(1.2); border-top:1px solid rgba(2,6,23,0.06); box-shadow:0 -10px 40px rgba(2,6,23,0.08); }
        detailPanel.detail-panel.open.dp-mode-bar:has(.detail-tab.active[data-tab="info"]) .detail-panel-body { padding-bottom:calc(10px + 64px + env(safe-area-inset-bottom)) !important; }
        
        detailPanel .dp-mobile-actionbar .dp-actions-grid { display:flex; flex-wrap:nowrap; gap:10px; width:100%; overflow-x:auto; -webkit-overflow-scrolling:touch; scrollbar-width:none; }
        detailPanel .dp-mobile-actionbar .dp-actions-grid::-webkit-scrollbar { display:none; }
        
        /* Nút trong Navigation Bar */
        detailPanel .dp-mobile-actionbar .dp-action-btn { flex:0 0 auto; height:50px; padding:6px 14px; border-radius:14px; background:#ffffff; border:1px solid rgba(2,6,23,0.08); box-shadow:0 4px 12px rgba(2,6,23,0.04); display:flex; flex-direction:row; align-items:center; justify-content:flex-start; gap:10px; cursor:pointer; }
        detailPanel .dp-mobile-actionbar .dp-action-btn:active { background:#f8fafc; border-color:rgba(2,6,23,0.12); }
        
        detailPanel .dp-mobile-actionbar .dp-action-btn i { font-size:18px; width:20px; text-align:center; color:rgba(15,23,42,0.7); }
        detailPanel .dp-mobile-actionbar .dp-action-btn[data-action="inout"] i, detailPanel .dp-mobile-actionbar .dp-action-btn[data-action="move"] i { color:#F59E0B; }
        detailPanel .dp-mobile-actionbar .dp-action-btn[data-action="checkin"] i { color:#10B981; }
        detailPanel .dp-mobile-actionbar .dp-action-btn[data-action="checkout"] i { color:#EF4444; }
        detailPanel .dp-mobile-actionbar .dp-action-btn[data-action="inventory"] i { color:#0f766e; }
        detailPanel .dp-mobile-actionbar .dp-action-btn[data-action="qr"] i { color:#3b82f6; }
        detailPanel .dp-mobile-actionbar .dp-action-btn[data-action="photo"] i { color:#0284c7; }
        
        detailPanel .dp-mobile-actionbar .dp-action-btn .sub { display:none !important; }
        detailPanel .dp-mobile-actionbar.dp-actionbar--icononly .dp-action-btn .dp-label-group { display:none !important; }
        detailPanel .dp-mobile-actionbar.dp-actionbar--icononly .dp-action-btn { padding:6px 16px; min-width:46px; justify-content:center; }
      }

      /* ====== ASSISTIVE TOUCH FIX ====== */
      detailPanel.dp-mode-assistive .dp-assistive { display:flex; } /* Trả quyền display cho JS thông qua class cha */
      detailPanel .dp-assistive { position:fixed; z-index:2300; width:54px; height:54px; border-radius:999px; background:linear-gradient(135deg, rgba(88,101,242,0.96), rgba(121,75,196,0.96)); box-shadow:0 12px 32px rgba(88,101,242,0.35); border:1px solid rgba(255,255,255,0.25); display:none; align-items:center; justify-content:center; color:#fff; cursor:pointer; user-select:none; touch-action:none; }
      detailPanel .dp-assistive i { font-size:20px; text-shadow:0 2px 4px rgba(0,0,0,0.2); }
      detailPanel .dp-assistive-menu { position:fixed; z-index:2310; width:min(340px, calc(100vw - 32px)); border-radius:20px; background:rgba(255,255,255,0.90); backdrop-filter:blur(24px) saturate(1.2); -webkit-backdrop-filter:blur(24px) saturate(1.2); border:1px solid rgba(2,6,23,0.08); box-shadow:0 20px 60px rgba(2,6,23,0.2); padding:14px; display:none; }
      detailPanel .dp-assistive-menu .dp-assistive-title { font-weight:950; font-size:12px; color:rgba(15,23,42,0.75); margin:0 4px 12px; text-transform:uppercase; letter-spacing:0.5px; }
      ...
      detailPanel .dp-assistive-menu .dp-action-btn[data-action="qu-weight"] { background:#fffbeb; border-color:#f59e0b; }
      detailPanel .dp-assistive-menu .dp-action-btn[data-action="qu-weight"] i { background:#fef3c7; color:#d97706; }
      detailPanel .dp-assistive-menu .dp-action-btn[data-action="qu-design"] { background:#eff6ff; border-color:#3b82f6; }
      detailPanel .dp-assistive-menu .dp-action-btn[data-action="qu-design"] i { background:#dbeafe; color:#2563eb; }
      detailPanel .dp-assistive-menu .dp-action-btn[data-action="qu-lifecycle"] { background:#ecfdf5; border-color:#10b981; }
      detailPanel .dp-assistive-menu .dp-action-btn[data-action="qu-lifecycle"] i { background:#d1fae5; color:#059669; }
  ...
  function DetailPanelAssistiveTouch(options){
    ...
    function openSettings(){
      if(!panelEl) return;
      settingsSheet = buildSettingsSheet(panelEl);
    ...
    function rebuildActionbar(){
      if(!panelEl) return;
      const obj = ensureActionbar(panelEl);
      actionbar = obj.bar;
    ...
    function rebuildAssistiveMenu(){
      if(!panelEl) return;
      const obj = ensureAssistive(panelEl);
    ...
    function refreshVisibility(){
      ...
      const tab = getActiveTab(panelEl);
      const isInfo = (tab === 'info');

      if(!open || !mobile || !isInfo || !cfg.enabled){
        if(panelEl.classList.contains('dp-mode-bar')) panelEl.classList.remove('dp-mode-bar');
        if(panelEl.classList.contains('dp-mode-assistive')) panelEl.classList.remove('dp-mode-assistive');
        if(actionbar) hideEl(actionbar);
        if(assistiveBtn) hideEl(assistiveBtn);
        closeAssistiveMenu();
        if(settingsSheet) closeSettingsSheet(settingsSheet);
        setBodyPaddingForBar(panelEl, false);
        return;
      }

      if(cfg.mode === 'assistive'){
        if(panelEl.classList.contains('dp-mode-bar')) panelEl.classList.remove('dp-mode-bar');
        if(!panelEl.classList.contains('dp-mode-assistive')) panelEl.classList.add('dp-mode-assistive');
        ...
        if(panelEl.classList.contains('dp-mode-assistive')) panelEl.classList.remove('dp-mode-assistive');
        if(!panelEl.classList.contains('dp-mode-bar')) panelEl.classList.add('dp-mode-bar');
        
        if(assistiveBtn) hideEl(assistiveBtn);
        closeAssistiveMenu();
        if(actionbar) showEl(actionbar, 'flex');
        setBodyPaddingForBar(panelEl, true);
      }
