...
(function (global) {
  ...
  function PhotoManagerModule() {
    ...
    // Cache + incremental render (v8.4.6-2-3)
    ...
    console.log('[PhotoManager] v8.4.6-2-3 created');
  }
  ...
  PhotoManagerModule.prototype._mount = function () {
    ...
    try {
      ...
      if (DevPS) {
        if (DevPS.tableName === 'devicephotos') DevPS.tableName = 'device_photos';
      }
    } catch (e) {}

    var self = this;

    // v8.4.6-2-3: Không auto reload danh sách ảnh khi có event upload/update.
...
;
  ...
  /* ── _renderGrid ─────────────────────────────────────────── */
  // v8.4.6-2-3: render grid theo lát, tránh render quá nhiều ảnh 1 lần
...
;
  ...
  PhotoManagerModule.prototype._bindAll = function () {
    ...
    /* Upload button – mở popup upload, không đóng Photo Manager (v8.4.6-2-3) */
    

    // Refresh button (v8.4.6-2-3)
    ...
    // Click device link -> mở nhanh Detail Panel (v8.4.6-2-3)
  ...
  /* CACHE + THUMB + INCREMENTAL RENDER (v8.4.6-2-3) */
  ...
  PhotoManagerModule.prototype._updateThumbColumns = async function (photoId, payloadA, payloadB) {
    ...
    if (table === 'devicephotos') table = 'device_photos';
  ...
  // =========================
  // SUPABASE CLIENT + DEBUG (v8.4.6-2-3)
  // =========================
  ...
  function _pmDbg() {
    ...
    try {
      ...
      a.unshift('[PhotoManager v8.4.6-2-3]');
...
/* ──────────────────────────────────────────────────────────
  ...
  console.log('[PhotoManager] v8.4.6-2-3 registered as window.PhotoManager');
