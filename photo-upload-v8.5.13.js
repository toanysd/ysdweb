...
(function (global) {
  ...
  var TMPL_MAIN = [
    ...
    '      <div class="pu-confirm-title" style="margin-bottom:8px;">Chuẩn bị Mở Camera</div>',
    '      <div class="pu-confirm-msg" id="puSecOverlayMsg" style="margin-bottom:16px;">',
    '        Hệ thống đang cấu hình Camera Trước để chụp ảnh báo cáo (theo chính sách an ninh).<br><br><b>💡 Xin lưu ý:</b> Vui lòng bấm <b>Cho phép</b> nếu HĐH yêu cầu quyền sử dụng máy ảnh!',
    '      </div>',
    '      <div class="pu-camera-video-wrap" style="height:150px; margin-bottom:14px; background:#000; border-radius:8px; overflow:hidden; position:relative;">',
    '        <video id="puSecFrontVideo" class="pu-camera-video" autoplay playsinline muted></video>',
    '        <div class="pu-spinner" id="puSecSpinner" style="position:absolute; top:50%; left:50%; margin:-10px 0 0 -10px;"></div>',
    '      </div>',
    '      <div class="pu-confirm-btns" style="justify-content:center;">',
    '        <button class="pu-confirm-btn pu-confirm-primary" id="puSecOverlayAccept" type="button" style="flex:1;">Đang tải xin vui lòng đợi...</button>',
    '        <button class="pu-confirm-btn pu-confirm-cancel" id="puSecOverlayCancel" type="button" style="flex:1;">Hủy chụp ảnh</button>',
    '      </div>',
    '    </div>',
    '  </div>',
  ...
  PhotoUploadModule.prototype._bindStatic = function () {
    ...
    if (btnAsTakePhoto) {
      btnAsTakePhoto.addEventListener('click', function() {
        ...
        if (secAcceptBtn) {
           secAcceptBtn.disabled = true;
           secAcceptBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Đang nạp máy ảnh trước...';
        }
        
        var video = document.getElementById('puSecFrontVideo');
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
           if (secAcceptBtn) { secAcceptBtn.disabled = false; secAcceptBtn.innerHTML = 'Tiếp tục Mở Camera Sau'; }
           return;
        }
        
        self._securityPhotoBlob = null;
        navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user' }, audio: false })
          .then(function(stream) {
             ...
             var timeout = setTimeout(function() {
               if (!isCaptured) { 
                 ...
                 if (secAcceptBtn) { secAcceptBtn.disabled=false; secAcceptBtn.innerHTML='Tiếp tục Camera'; }
               }
             }, 8000);
             ...
             var poll = setInterval(function() {
               ...
               if (video && video.readyState >= 2 && video.videoWidth > 0) {
                 ...
                 try {
                   ...
                   c.toBlob(function(b){
                      self._securityPhotoBlob = b;
                      cleanup();
                      if (secAcceptBtn) {
                        secAcceptBtn.disabled = false;
                        secAcceptBtn.innerHTML = '<i class="fas fa-check-circle"></i> Đã báo cáo ẩn xong.<br><b>Bấm để Mở Camera Sản Phẩm!</b>';
                      }
                   }, 'image/jpeg', 0.9);
                 } catch(e) { 
                   cleanup(); 
                   if (secAcceptBtn) { secAcceptBtn.disabled=false; secAcceptBtn.innerHTML='Tiếp tục'; }
                 }
               }
             }, 150);
          })
          .catch(function(e) {
             ...
             if (secAcceptBtn) { secAcceptBtn.disabled = false; secAcceptBtn.innerHTML = 'Bỏ qua (Do bị từ chối) & Mở Camera'; }
          });
      });
    }

    if (secAcceptBtn) secAcceptBtn.addEventListener('click', function() {
        if (secOv) secOv.classList.add('pu-hidden');
        if (fileInputCamera) fileInputCamera.click();
    });

    if (secCancelBtn) secCancelBtn.addEventListener('click', function() {
  ...
  PhotoUploadModule.prototype._uploadSecurityBlob = function (blob, code, cb) {
    ...
    client.storage.from(bucket).upload(path, blob)
      .then(function(res) {
        if (res.error) {
          console.warn('[PhotoUpload] Failed to upload security blob to audit/:', res.error, bucket);
          alert('Lỗi quyền hệ thống: Upload ảnh bảo mật thất bại!\nCấu hình bảo mật thư mục audit/ thuộc Bucket [' + bucket + '] đang chặn việc lưu file. Log: ' + res.error.message);
          return cb(null);
        }
