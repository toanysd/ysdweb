...
var VERSION = 'v8.5.0-4';
var STORAGE_KEY = 'dp_extended_editor_drafts_v8_5_0_4';
...
var FIELD_PREF = {
  ...
  trays: ['TrayID','TrayCode','TrayName','TrayCapacity','Notes'],
  tray: ['TrayID','TrayCode','TrayName','TrayCapacity','Notes'],
...
function buildContext(dp) {
  ...
  var trayId = t((baseRow && baseRow.TrayID) || item.TrayID);
...
function render(dp, container, state) {
  ...
  html += '</div></aside><section class="ext-workspace-right"><section class="ext-section ext-detail-card is-open ' + ((state.editor && state.editor.sectionKey === activeSection.key) ? 'is-editing ' : '') + ((draftsOfSection(state.drafts, activeSection.key).length) ? 'has-draft' : '') + '" data-section="' + esc(activeSection.key) + '">';
  html += '<div class="ext-section-header ext-detail-head"><div class="ext-section-header-left"><div class="ext-section-icon"><i class="' + esc((SECTION_META[activeSection.key] || {}).icon || 'fas fa-folder') + '"></i></div><div class="ext-section-title"><div class="ext-section-title-jp">' + esc((SECTION_META[activeSection.key] || {}).jp || activeSection.key) + '</div><div class="ext-section-title-vi">' + esc((SECTION_META[activeSection.key] || {}).vi || activeSection.key) + '</div></div></div><div class="ext-section-header-right">' + (draftsOfSection(state.drafts, activeSection.key).length ? '<span class="ext-section-badge draft">Nháp ' + draftsOfSection(state.drafts, activeSection.key).length + '</span>' : '') + '</div></div>';
...
function bind(dp, container) {
  if (!container) return;
  var state = container.dpExtState || { editor:null, store:loadStore(), drafts:[], isSaving:false, ctx:null, sections:{}, activeSection:'' };
