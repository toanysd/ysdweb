...
============================================================================ */
console.log('✅ results-card-renderer-v8.4.3 LOADED');
...
class ResultsCardRenderer {
    ...
    init() {
        ...
        // Zoom button handler (mở nhanh ảnh thật)
        // Fix: luôn lấy ảnh active mới nhất (không phụ thuộc is_thumbnail)
        // + mở popup ngay khi click để tránh bị chặn popup do await
        document.addEventListener('click', (e) => {
            ...
            // QUAN TRỌNG: dùng đúng ID đang gắn vào card (device_id trong DB thường là ID này)
            ...
            // Mở popup ngay lập tức (nếu browser cho phép)
            let pop = null;
            try {
                pop = window.open('about:blank', '_blank', 'noopener');
                if (pop && pop.document && pop.document.body) {
                    pop.document.title = String(title || 'Photo');
                    pop.document.body.style.fontFamily = 'system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial';
                    pop.document.body.style.padding = '16px';
                    pop.document.body.innerHTML = '<div style="font-weight:800;color:#0f172a">Loading...</div><div style="margin-top:6px;color:#64748b">Đang mở ảnh...</div>';
                }
            } catch (ePop) {
                pop = null;
            }
            ...
            (async () => {
                try {
                    ...
                    // 1) Ưu tiên: listForDevice (có order created_at desc)
                    ...
                    // 2) Fallback: getPhotos(filters) -> { data: [...] }
                    ...
                    // 3) Fallback cuối: thumbnail (có thể null nếu chưa set thumb)
                    else if (typeof window.DevicePhotoStore.getThumbnailForDevice === 'function') {
                    ...
                    // Nếu popup mở được thì chuyển sang ảnh
                    if (pop && !pop.closed) {
                        pop.location.href = fullUrl;
                        try { pop.focus(); } catch (eFocus) {}
                        return;
                    }

                    // Nếu popup bị chặn: mở ngay trên tab hiện tại để chắc chắn xem được
                    window.location.href = fullUrl;
                } catch (err) {
                    console.warn('Open full image error:', err);
                    try {
                        if (pop && !pop.closed) {
                            pop.document.body.innerHTML = '<div style="font-weight:900;color:#b91c1c">Không mở được ảnh</div><div style="margin-top:6px;color:#64748b">Vui lòng thử lại</div>';
                        }
                    } catch (e2) {}
                    alert('Không mở được ảnh thật. Vui lòng thử lại');
                }
            })();
        });
    ...
    createCard(item) {
        ...
        card.innerHTML = `
            ...
            <div class="card-thumbnail">
                ...
                <button class="image-zoom-btn" data-id="${itemId}" title="Phóng to ảnh" onclick="event.stopPropagation()">
    ...
    const imgs = this.container.querySelectorAll('img.card-thumb-img[data-devicetype][data-deviceid]');
        for (const img of imgs) {
            ...
            // tạo promise load mới
            const p = (async () => {
            try {
                let url = '';

                // Ưu tiên: API trả về thumb URL và tự tạo thumb nếu thiếu
                if (typeof window.DevicePhotoStore.getThumbnailUrl === 'function') {
                    url = await window.DevicePhotoStore.getThumbnailUrl(dt, did);
                } else {
                    // Fallback: chỉ lấy các field thumb (KHÔNG lấy publicurl)
