...
(function (global) {
  ...
  var TMPL_MAIN = [
    ...
    '        </div>',
    '      </div>',
    '      <button class="pu-quick-name-btn" id="puQuickNameBtn"><i class="fas fa-bolt"></i> 無名でクイック入力 / Nhập nhanh không tên</button>',
    '    </div>',
    /* Code + Dims */
  ...
  function PhotoUploadModule() {
    ...
    console.log('[PhotoUpload] v8.4.3-4-1 created');
  }
  ...
  PhotoUploadModule.prototype._mount = function () {
    ...
    this._bindStatic();
    this._loadEmployees();
    console.log('[PhotoUpload] v8.4.3-4-1 Mounted');
  };
  ...
  PhotoUploadModule.prototype._reset = function () {
    ...
    var ids = ['puDeviceCode','puDimensions','puNotes','puEmailInput'];
    ...
    ['puDeviceCode','puDimensions'].forEach(function(id){
      var el=document.getElementById(id); if(el) el.classList.remove('pu-input-auto');
    });
  ...
  PhotoUploadModule.prototype._processImage = function (blob, transforms, resizeMode) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      img.onload = function () {
        ...
        /* Không có transform + original → trả về blob gốc */
        if (angle === 0 && !flipH && !flipV && resizeMode === 'original') {
          resolve(blob); return;
        }
        ...
        /* Kích thước canvas sau khi xoay */
        var cw = Math.round(ow * cos + oh * sin);
        var ch = Math.round(ow * sin + oh * cos);
        /* Scale theo resizeMode */
        var scaleDown = 1;
        var quality   = 0.92;
        if (resizeMode === 'hd') {
          scaleDown = Math.min(1920 / cw, 1080 / ch, 1);
          quality   = 0.90;
        } else if (resizeMode === 'compact') {
          scaleDown = Math.min(1200 / Math.max(cw, ch), 1);
          quality   = 0.72;
        }
        cw = Math.round(cw * scaleDown);
        ch = Math.round(ch * scaleDown);

        var drawScale = scaleDown;
        ...
        ctx.rotate(rad);
        ctx.drawImage(img, -(ow * drawScale) / 2, -(oh * drawScale) / 2, ow * drawScale, oh * drawScale);
        ctx.restore();
  ...
  PhotoUploadModule.prototype._validate = function () {
    ...
    if (!code.trim()) {
      ...
      var ci2 = document.getElementById('puDeviceCode');
      if (ci2) ci2.classList.remove('pu-error');
    }
    if (!this._photos.length) {
  ...
  PhotoUploadModule.prototype._finalize = function (results, errors) {
    ...
    if (errors.length && !results.length) {
      this._showResult(false, 'エラー: ' + (errors[0] && errors[0].message || 'Unknown error'));
    } else {
      ...
      /* Tự đóng sau khi gửi thành công 
      var self = this;
  ...
  console.log('[PhotoUpload] v8.4.3-4-1 registered as window.PhotoUpload');
