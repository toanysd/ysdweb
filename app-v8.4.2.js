...
class App {
  ...
  async init() {
    ...
    this.forceSidebarForViewport();
    window.addEventListener('resize', () => this.forceSidebarForViewport());
...
/**
  ...
  toggleSidebar(){
    ...
    try{ localStorage.setItem(SIDEBARSTATEKEY, isCollapsed ? '1' : '0'); }catch(e){}
  }

  forceSidebarForViewport(){
    const sidebar = document.getElementById('sidebar');
    if(!sidebar) return;
    ...
    if(isMobile){
      // Mobile: luôn đóng hẳn, không để trạng thái "collapsed" từ desktop làm vướng
      sidebar.classList.remove('collapsed');
      sidebar.classList.add('mobile-closed');
    }else{
      // Desktop/Tablet: bỏ trạng thái mobile và restore theo localStorage như cũ
  ...
  openPhotoUploadQuick() {
    // Ưu tiên tool mới (PhotoUploadTool)
    ...
    // Tương thích tool cũ (PhotoAuditTool)
    if (window.PhotoAuditTool) {
      ...
      if (typeof window.PhotoAuditTool.open === 'function') {
        window.PhotoAuditTool.open({ autoOpenFile: true });
        return;
      }
    }

    alert('Chưa có công cụ Upload ảnh. Cần load photo-upload-tool-v8.4.0.js');
  }
  ...
  ensureMobilePhotoMenu() {
    ...
    // Fallback: gắn vào mobileNavbar container nếu tồn tại
    const mobile = document.getElementById('mobileNavbar');
    if (mobile && !document.getElementById('mobile-photo-menu-btn')) {
      const btn = document.createElement('button');
      btn.id = 'mobile-photo-menu-btn';
      btn.type = 'button';
      btn.textContent = 'Ảnh';
      btn.className = 'mcs-photo-menu-btn secondary';
      btn.innerHTML = `
        <span class="mcs-bilingual">
          <span class="mcs-ja">写真</span>
          <span class="mcs-vi">Ảnh</span>
    ...
    switch(action) {
      case 'checkin':
        if (window.CheckInOut && typeof window.CheckInOut.openModal === 'function') {
          ...
          alert('Chưa nạp module CheckInOut (checkin-checkout-v8.4.3.js)');
        }
        break;

      case 'checkout':
        if (window.CheckInOut && typeof window.CheckInOut.openModal === 'function') {
          window.CheckInOut.openModal('check-out', item);
        } else {
          alert('Chưa nạp module CheckInOut (checkin-checkout-v8.4.3.js)');
        }
        break;
      ...
      case 'qr':
        ...
        break;
      case 'photo':
        this.openPhotoAuditForItem(item);
        break;
