  ...
  class DetailPanel {
    ...
    bindEvents() {
      ...
      this.panel.addEventListener('click', async (e) => {
        ...
        // (A) Bấm nút “mở ảnh thật”
        const btnOpenFull = e.target && e.target.closest ? e.target.closest('[data-dp-photo-openfull="1"]') : null;
        if (btnOpenFull) {
          e.preventDefault();
          e.stopPropagation();
  ...
  try {
    ...
    async hydrateDesktopPhotoPreviewFromSupabase() {
      try {
        ...
        // Tìm đúng cái <img> mình vừa gắn data-dp-thumb-img
        ...
        if (!window.DevicePhotoStore || typeof window.DevicePhotoStore.listForDevice !== 'function') return

        // Nếu store có ensureReady thì gọi (file của bạn có chỗ dùng ensureReady ở tab Photos)
        try {
          if (typeof window.DevicePhotoStore.ensureReady === 'function') window.DevicePhotoStore.ensureReady()
        } catch (e) {}

        // Lấy deviceId giống cách tab Photos đang làm
        ...
        // Lấy 1 row đại diện để có photoId + fullUrl
        if (window.DevicePhotoStore && typeof window.DevicePhotoStore.getThumbnailForDevice === 'function') {
          picked = await window.DevicePhotoStore.getThumbnailForDevice(this.currentItemType, String(deviceId), { state: 'active' });
        }
        if (!picked && window.DevicePhotoStore && typeof window.DevicePhotoStore.getLatestActivePhotoForDevice === 'function') {
          picked = await window.DevicePhotoStore.getLatestActivePhotoForDevice(this.currentItemType, String(deviceId));
        }

        // Thumb URL để hiển thị (chỉ thumb)
        let thumbUrl = '';
        if (window.DevicePhotoStore && typeof window.DevicePhotoStore.getThumbnailUrl === 'function') {
          thumbUrl = await window.DevicePhotoStore.getThumbnailUrl(this.currentItemType, String(deviceId));
        }

        // Full URL để mở nhanh khi bấm
    ...
    renderDesktopPhotoPreview(item) {
      ...
      return `
        <div class="modal-section">
          ...
          <div style="position:relative;border-radius:12px;overflow:hidden;border:1px solid rgba(15,23,42,0.10);background:#fff">
            <img data-dp-thumb-img="1"
                src=""
                alt=""
                style="width:100%;height:auto;display:none;cursor:zoom-in"
                onerror="this.style.display='none'"/>

            <div style="position:absolute;top:10px;right:10px;display:flex;gap:8px;z-index:3">
              <button type="button" data-dp-photo-openfull="1" title="Mở ảnh thật"
            ...
            <div data-dp-thumb-placeholder="1"
                style="padding:14px;color:#64748b;font-weight:700;display:block;cursor:pointer"></div>
          </div>
    ...
    renderMoldBasicInfoFull(item) {
      ...
      const fields = [
        ...
        { label: this.biLabel('寸法', 'Kích thước khuôn (LxWxH)'), rawValue: lwh },
        { label: this.biLabel('金型重量', 'Khối lượng khuôn'), rawValue: this.escapeHtml(weightTxt) },
