...
(function() {
    ...
    async function fetchLatestSha() {
        try {
            const apiUrl = (typeof global !== 'undefined' && global.MCS_API_BASE_URL) 
                ? global.MCS_API_BASE_URL 
                : 'https://ysd-moldcutter-backend.onrender.com';
            const res = await fetch(`${apiUrl}/api/latest-sha`);
            if (res.ok) {
                const data = await res.json();
                if (data.sha) currentGithubSha = data.sha;
                console.log('✅ Resolved latest SHA:', currentGithubSha);
            }
        } catch (e) {
            console.warn('⚠️ Could not fetch latest SHA, falling back to main');
        }
    }
