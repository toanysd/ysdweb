...
(function (global) {
  ...
  function PhotoManagerModule() {
    ...
    // Cache + incremental render (v8.4.6-1)
    this._photoCache = new Map(); // key -> { photos: [], ts: number }
    this._cacheTtlMs = 30 * 60 * 1000; // TTL 30 phút
    ...
    console.log('[PhotoManager] v8.4.6-1 created');
  }
  ...
  PhotoManagerModule.prototype._mount = function () {
    ...
    // v8.4.6-1: Không auto reload danh sách ảnh khi có event upload/update.
...
;
  ...
  /* ── _renderGrid ─────────────────────────────────────────── */
  // v8.4.6-1: render grid theo lát, tránh render quá nhiều ảnh 1 lần
  PhotoManagerModule.prototype._renderGridSlice = function (containerId, startIdx, endIdx, append) {
    ...
    for (var i = startIdx; i < endIdx; i++) {
      ...
      var rawSize = Number(p.file_size || p.filesize || p.fileSize || 0);
    ...
    container.querySelectorAll('.pm-item').forEach(function (el) {
      ...
      el.addEventListener('click', function (e) {
        ...
        self._openLightbox(idx);
      });

      el.addEventListener('dblclick', function () {
        var rec = self._filtered[idx];
        if (rec) self._openInfoPanel(rec);
      });

      el.addEventListener('contextmenu', function (e) {
        e.preventDefault();
        self._showContextMenu(e, self._filtered[idx]);
      });
...
;
  ...
  PhotoManagerModule.prototype._renderDetailSlice = function (startIdx, endIdx, append) {
    ...
    for (var i = startIdx; i < endIdx; i++) {
      ...
      var rawSize = Number(p.filesize || 0);
      var size = (DevPS && rawSize > 0 && typeof DevPS.formatBytes === 'function') ? DevPS.formatBytes(rawSize) : '';

      var dtype = (p.devicetype || p.deviceType || '').toString().toLowerCase();
      var did = (p.deviceid || p.deviceId || '').toString();
      var typeLabel = (dtype === 'mold') ? 'Khuôn' : (dtype === 'cutter') ? 'Dao cắt' : dtype;
...
;
  ...
  PhotoManagerModule.prototype._updateStorageBar = function () {
    ...
    var calcFromLoaded = function () {
      var totalSize = self._allPhotos.reduce(function (acc, p) { return acc + (Number(p.file_size) || 0); }, 0);
  ...
  PhotoManagerModule.prototype._showContextMenu = function (e, record) {
    ...
    var items = [];
    if (!isTrash) {
      items.push({ icon: 'fa-eye',          label: 'プレビュー / Xem',             cls: '',             fn: function () { self._openLightbox(self._filtered.indexOf(record)); } });
  ...
  PhotoManagerModule.prototype._renderLightboxSlide = function () {
    ...
    var rawSize      = Number(p.file_size) || 0;
  ...
  PhotoManagerModule.prototype._openStorageModal = function () {
    ...
    var fallbackStats = {
      ...
      totalSize: self._allPhotos.reduce(function (a, p) { return a + (Number(p.file_size) || 0); }, 0),
      trashSize: self._allPhotos.filter(function (p) { return p.state === 'trash'; }).reduce(function (a, p) { return a + (Number(p.file_size) || 0); }, 0)
    };
  ...
  PhotoManagerModule.prototype._bindAll = function () {
    ...
    /* Upload button – mở popup upload, không đóng Photo Manager (v8.4.6-1) */
    

    // Refresh button (v8.4.6-1)
    ...
    // Click device link -> mở nhanh Detail Panel (v8.4.6-1)
  ...
  /* CACHE + THUMB + INCREMENTAL RENDER (v8.4.6-1) */
  ...
  PhotoManagerModule.prototype._getThumbUrl = function (p) {
    ...
    // Thumb đúng chuẩn do DevicePhotoStore v8.4.3-7 tạo ra: đường dẫn có 'thumb_'
  ...
  // ĐOẠN MỚI (đồng bộ tuyệt đối với DevicePhotoStore)
  PhotoManagerModule.prototype._buildThumbStoragePath = function (record) {
    ...
    // Đồng bộ tuyệt đối với DevicePhotoStore v8.4.3-7: <dir>/thumb_<core>.jpg
  ...
  PhotoManagerModule.prototype._updateThumbColumns = async function (photoId, payloadA, payloadB) {
    var DevPS = global.DevicePhotoStore;
    var client = DevPS && typeof DevPS.getClient === 'function' ? DevPS.getClient() : null;
    if (!client) throw new Error('No supabase client');
    var table = DevPS.tableName || 'devicephotos';

    // Chỉ dùng snake_case theo quy ước Supabase
  ...
  PhotoManagerModule.prototype._createThumbForRecord = async function (rec, cfg) {
    ...
    var client = (typeof DevPS.getClient === 'function') ? DevPS.getClient() : null;
    if (!client) throw new Error('No supabase client');
...
/* ──────────────────────────────────────────────────────────
  ...
  console.log('[PhotoManager] v8.4.6-1 registered as window.PhotoManager');
