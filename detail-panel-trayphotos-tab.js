...
(function (global) {
  ...
  function getDeviceId(dp) {
    try {
      ...
      if (!it) return '';
      if (dp.currentItemType === 'tray') return trimStr(it.TrayID || it.ID || '');
      return trimStr(it.TrayID || it.TrayCode || '');
    } catch (e) {
      return '';
    }
  }
  ...
  function getDeviceCode(dp) {
    try {
      var it = dp && dp.currentItem ? dp.currentItem : null;
      if (!it) return '';
      if (dp.currentItemType === 'tray') return trimStr(it.TrayCode || it.ID || '');
      return trimStr(it.TrayCode || it.TrayID || '');
    } catch (e) {
      return '';
    }
  }
  ...
  function injectStyles() {
    if (document.getElementById('pt-styles-tray-v8431')) return;
    var st = document.createElement('style');
    st.id = 'pt-styles-tray-v8431';
    st.textContent = [
      /* Toolbar */
  ...
  ════════════════════════════════════════ */
    ...
    if (!deviceId || deviceId === '-' || deviceId === '---') {
      return (
        '<p class="no-data" style="padding:14px">' +
          '<span class="pt-ja">関連するトレイがありません</span> ' +
          '<span class="pt-vi">Không có khay liên kết.</span>' +
        '</p>'
      );
    }
    ...
    if (!dp._ptStoreBound) {
      ...
      function _tryRefresh() {
        try {
          ...
          var h = dp.panel.querySelector('[data-tab-content="trayphotos"] [data-dp-photo-host]');
  ...
  async function loadIntoHost(dp, host, opts) {
    ...
    try {
      if (host && host.isConnected === false && dp && dp.panel) {
        var h2 = dp.panel.querySelector('[data-tab-content="trayphotos"] [data-dp-photo-host]');
        if (h2) host = h2;
      }
    } catch (eH) { /* ignore */ }
  ...
  try {
    ...
    global.DetailPanelTabModules.trayphotos = TrayPhotosTabModule;
  } catch (e) { /* ignore */ }
  ...
  try {
    document.dispatchEvent(new CustomEvent('detail-panel-trayphotos-tab-module-ready', { detail: { version: VERSION } }));
  } catch (e3) {}

  try { console.log('DetailPanel TrayPhotosTabModule ' + VERSION + ' loaded'); } catch (e4) {}
