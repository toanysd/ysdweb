    ...
    ensurePanelLayout() {
      this.panel.innerHTML = `
        <div class="detail-panel-header">
          ...
          <button class="detail-panel-back" aria-label="Back" type="button" style="display:none; margin-right:8px;">
    ...
    ensurePreviewStyles() {
      try {
        ...
        st.textContent = `
          ...
          /* v8.4.3-2: Preview header color by device type */
          ...
          .dp-preview-modal.dp-preview-type--mold .dp-preview-header {
            background: linear-gradient(90deg, rgba(15,118,110,1) 0%, rgba(20,184,166,1) 100%);
            color:#ffffff;
          }
          .dp-preview-modal.dp-preview-type--cutter .dp-preview-header {
          ...
          /* v8.4.3-2: Hero in preview should be softer (avoid strong orange/teal) */
          .dp-preview-modal .dp-hero {
            border-radius: 14px;
            border: 1px solid rgba(2,6,23,0.08);
          }
          .dp-preview-modal .dp-hero--mold {
            background: linear-gradient(135deg, rgba(204,251,241,0.75) 0%, rgba(240,253,250,0.92) 60%);
            border-color: rgba(13,148,136,0.16);
          }
          .dp-preview-modal .dp-hero--cutter {
            background: linear-gradient(135deg, rgba(255,237,213,0.75) 0%, rgba(255,251,235,0.92) 60%);
            border-color: rgba(234,88,12,0.14);
          }
          .dp-preview-modal .dp-hero-line1,
          .dp-preview-modal .dp-hero-line2,
          .dp-preview-modal .dp-hero-k,
          .dp-preview-modal .dp-hero-v {
            color: rgba(2,6,23,0.90);
          }

          /* v8.4.3-2: Ensure bilingual labels show JP on top, VI below (preview) */
          .dp-preview-modal .dp-label-jp,
          .dp-preview-modal .dp-label-vi {
            display:block;
            line-height:1.18;
          }
          .dp-preview-modal .dp-label-vi {
          ...
          /* v8.4.3-2: Note style for reference text */
          .dp-preview-modal .info-message.dp-note {
            margin-top:10px;
            padding:10px 12px;
            border-radius:12px;
            font-size:12px;
          ...
          .dp-preview-modal.dp-preview-type--cutter .info-message.dp-note {
            color: rgba(124,45,18,0.95);
            background: rgba(234,88,12,0.07);
            border: 1px solid rgba(234,88,12,0.14);
            border-left: 3px solid rgba(234,88,12,0.36);
          }
          .dp-preview-modal.dp-preview-type--mold .info-message.dp-note {
            color: rgba(19,78,74,0.95);
            background: rgba(20,184,166,0.07);
            border: 1px solid rgba(13,148,136,0.14);
            border-left: 3px solid rgba(13,148,136,0.36);
          }
...
document.addEventListener('click', (e) => {
  try {
    ...
    renderDesktopHero(item, type) {
      ...
      return `
        <div class="${heroClass}">
          <div class="dp-hero-grid dp-hero-grid--with-thumb">
            <div class="dp-hero-col dp-hero-col-thumb">
              <img
                data-dp-preview-hero-thumb="1"
                data-dp-device-type="${isMold ? 'mold' : 'cutter'}"
                data-dp-device-id="${e(id)}"
                alt=""
              />
            </div>
    ...
    renderPreviewHero(item, type) {
      ...
      const idHtml = id && String(id).trim() && String(id).trim() !== "-" ? `<span class="dp-hero-id">${e(id)}</span>` : "";
      const nameHtml = name && String(name).trim() ? `<span class="dp-hero-dot">・</span><span class="dp-hero-name2">${e(name)}</span>` : "";
      const line2 = (idHtml || nameHtml) ? `${idHtml}${nameHtml}` : `<span class="dp-hero-id">-</span>`;

      const deviceType = isMold ? "mold" : "cutter";
      const deviceId = String(id || "").trim();

      const locationText = rackLayer ? `${rackLayer} ${String(loc || "").trim()}`.trim() : (String(loc || "-").trim());
      const companyText = companyInfo?.nameShort ? String(companyInfo.nameShort) : "-";
      const statusText = statusInfo?.textShort ? String(statusInfo.textShort) : (statusInfo?.text ? String(statusInfo.text) : "---");
      const confirmText = statusInfo?.lastConfirmDateText ? String(statusInfo.lastConfirmDateText) : "-";

      // 1x1 pixel để tránh icon lỗi, đồng thời giữ chỗ cho khung ảnh
        ...
        <div class="dp-hero-grid dp-preview-hero-grid">
          ...
          <div class="dp-preview-hero-right">
            <div class="dp-preview-hero-top">
              <div class="dp-hero-line1">${e(line1)}</div>
              <div class="dp-hero-line2">${line2}</div>
            </div>

            <div class="dp-preview-hero-bottom">
              <div class="dp-preview-hero-kvgrid">
                <div class="dp-hero-kv">
                  <span class="dp-hero-k">Vị trí</span>
                  <span class="dp-hero-v">${e(locationText)}</span>
                </div>

                <div class="dp-hero-kv">
                  <span class="dp-hero-k">Công ty</span>
                  <span class="dp-hero-v ${companyInfo?.needsHighlight ? "dp-hero-v--warn" : ""}">${e(companyText)}</span>
                </div>

                <div class="dp-hero-kv">
                  <span class="dp-hero-k">Trạng thái</span>
                  <span class="dp-hero-v">${e(statusText)}</span>
                </div>

                <div class="dp-hero-kv">
                  <span class="dp-hero-k">Xác nhận</span>
              ...
              ${returning ? `
                <div class="dp-hero-kv dp-hero-kv--warn">
                  <span class="dp-hero-k">Trả khuôn</span>
                  <span class="dp-hero-v">${e(returning)}</span>
                </div>
              ` : ""}
              ...
              ${disposing ? `
                <div class="dp-hero-kv dp-hero-kv--danger">
                  <span class="dp-hero-k">Huỷ khuôn</span>
                  <span class="dp-hero-v">${e(disposing)}</span>
                </div>
              ` : ""}
            </div>
      ...
      return `
        <div class="modal-section">
          ...
          <div class="info-grid-2col dp-kv-compact dp-kv-1col">
            <div class="info-item">
              <div class="info-label">${this.renderLabelHtml(this.biLabel('ステータス', 'Trạng thái'))}</div>
            ...
            <div class="info-item">
              <div class="info-label">${this.renderLabelHtml(this.biLabel('ロケーション', 'Vị trí'))}</div>
              <div class="info-value">${this.safeText(lastLocation?.NewLocation || lastLocation?.RackLayerID)}</div>
            </div>

            <div class="info-item">
              <div class="info-label">${this.renderLabelHtml(this.biLabel('ロケーション日時', 'Thời gian vị trí'))}</div>
              <div class="info-value">${this.safeText(this.formatDate(lastLocation?.Timestamp || lastLocation?.Date))}</div>
            </div>

            <div class="info-item">
              <div class="info-label">${this.renderLabelHtml(this.biLabel('出荷先', 'Điểm đến'))}</div>
              <div class="info-value">${this.safeText(this.getDestinationName(lastShip?.DestinationID || lastShip?.Destination))}</div>
            </div>

            <div class="info-item">
              <div class="info-label">${this.renderLabelHtml(this.biLabel('出荷日時', 'Thời gian vận chuyển'))}</div>
          ...
          <div style="position:relative;border-radius:12px;overflow:hidden;border:1px solid rgba(15,23,42,0.10);background:#fff;height:220px;min-height:220px">
            ...
            <div style="position:absolute;top:10px;right:10px;display:flex;gap:8px;z-index:3">
              <button type="button" data-dp-photo-download="1" title="Download ảnh thật"
                style="width:36px;height:36px;border-radius:999px;border:1px solid rgba(2,6,23,0.12);background:rgba(255,255,255,0.96);cursor:pointer">
                <i class="fas fa-download"></i>
              </button>


              <button type="button" data-dp-photo-trash="1" title="Xóa ảnh (vào thùng rác)"
                style="width:36px;height:36px;border-radius:999px;border:1px solid rgba(2,6,23,0.12);background:rgba(255,255,255,0.96);cursor:pointer;color:#b91c1c">
            ...
            <div data-dp-thumb-placeholder="1"
              ...
              <div data-dp-photo-upload="1" style="display:flex;gap:10px;align-items:center;max-width:360px">
                <i class="fas fa-camera" style="opacity:.65;font-size:22px"></i>
                <div>
                  <div style="font-size:13px">写真なし。クリックして撮影/アップロード</div>
                  <div style="font-size:11px;opacity:.90;font-weight:800">Chưa có ảnh, bấm để tải/chụp ảnh</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="modal-section location-section">
          ...
          <div class="location-content">
            ...
            <div style="display:grid;grid-template-columns:110px 1fr;gap:10px;align-items:start">
              <div style="position:relative;border-radius:12px;overflow:hidden;border:1px solid rgba(15,23,42,0.10);background:#fff;height:92px;min-height:92px">
                <img data-dp-thumb-img="1"
                     src=""
                     alt=""
                     ...
                     onerror="this.style.opacity='0'"/>
                <div data-dp-thumb-placeholder="1"
                     style="position:absolute;inset:0;padding:8px;color:#64748b;font-weight:800;display:flex;align-items:center;justify-content:center;cursor:pointer;text-align:center">
                  <div data-dp-photo-upload="1" style="display:flex;gap:6px;align-items:center">
                    <i class="fas fa-camera" style="opacity:.65"></i>
                    <div>
                      <div style="font-size:11px">写真なし。クリックして撮影/アップロード</div>
                      <div style="font-size:11px;opacity:.90;font-weight:900">
                        Chưa có ảnh, bấm để tải/ chụp ảnh
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="dp-storage-top" style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
                <div class="dp-storage-col">
                  <div class="dp-storage-label">${this.renderLabelHtml(this.biLabel('保管会社','Công ty'))}</div>
                  <div class="${companyBadgeCls}">${e(companyInfo.nameShort || '-')}</div>
                </div>
            ...
            <div class="dp-storage-raw">
              <div class="info-line">
                ...
                <span class="label">${this.renderLabelHtml(this.biLabel('位置','Vị trí'))}</span>
                <span class="value">${e(rackLocation)}</span>
              </div>
              ${rackNotes ? `
                <div class="info-line">
                  <i class="fas fa-sticky-note" style="opacity:.7"></i>
                  <span class="label">${this.renderLabelHtml(this.biLabel('メモ(棚)','Ghi chú giá'))}</span>
              ...
              ${layerNotes ? `
                <div class="info-line">
                  <i class="fas fa-sticky-note" style="opacity:.7"></i>
                  <span class="label">${this.renderLabelHtml(this.biLabel('メモ(段)','Ghi chú tầng'))}</span>
    ...
    renderMoldBasicInfoFull(item) {
      ...
      return `<div class="modal-section">
        ...
        ${this.renderInfoGrid(fields, 'dp-info-rows dp-keep-2col')}
      </div>`;
    }
    ...
    renderCutterProductReferenceSection(cutter) {
      ...
      return `
        <div class="modal-section">
          ...
          <div class="info-message dp-note">
