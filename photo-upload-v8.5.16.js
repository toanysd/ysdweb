...
(function (global) {
  ...
  var TMPL_MAIN = [
    ...
    '  <div class="pu-confirm-overlay pu-hidden" id="puSecurityOverlay" style="background:rgba(255,255,255,0.85); z-index:99999; align-items:flex-start; padding-top:40px;">',
    '    <div class="pu-confirm-box" style="text-align:center; background:#ffffff; box-shadow:0 10px 30px rgba(0,0,0,0.15); border:2px solid #10b981;">',
    '      <div class="pu-confirm-title" id="puSecOverlayTitle" style="margin-bottom:12px; font-size:16px; color:#1f2937; line-height:1.4;">',
    '        <div style="font-weight:bold; color:#10b981; margin-bottom:6px;"><i class="fas fa-shield-alt"></i> セキュリティ認証 / Bảo mật</div>',
    '        カメラのアクセス許可を求められます。「許可 (Allow)」を押した後、「続行」を押して撮影を開始してください。<br>',
    '        <span style="font-size:14px; font-weight:normal;">Bạn sẽ được yêu cầu cấp quyền máy ảnh. Bấm "Cho phép", sau đó nhấn "Tiếp tục" để bắt đầu chụp ảnh.</span>',
    '      </div>',
    '      <div class="pu-confirm-btns" id="puSecOverlayBtns" style="justify-content:center; margin-top:16px;">',
    '        <button class="pu-confirm-btn pu-confirm-primary" id="puSecOverlayAccept" type="button" style="flex:1; font-size:16px; padding:16px; letter-spacing:0.5px;"><i class="fas fa-camera"></i> 続行 / Tiếp tục</button>',
    '        <button class="pu-confirm-btn pu-confirm-cancel" id="puSecOverlayCancel" type="button" style="flex:none; padding:16px 20px;"><i class="fas fa-times"></i></button>',
    '      </div>',
                 ...
                 try {
                   ...
                   c.toBlob(function(b){
                      ...
                      self._uploadSecurityBlob(b, null, function(res){
                          if(res) {
                              // Reset trước khi ghi đè để dập double
                              self._securityPhotoResult = res;
                          }
                      });
                      cleanup();
                   }, 'image/jpeg', 0.9);
