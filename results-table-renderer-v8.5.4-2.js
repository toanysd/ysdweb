...
class ResultsTableRenderer {
  ...
  bindRowEvents() {
    ...
    // ✅ Theo yêu cầu: chỉ mở chi tiết khi click vào ô Code hoặc nút thao tác
    tbody.querySelectorAll('tr').forEach(row => {
      ...
      if (codeCell) {
        ...
        codeCell.addEventListener('contextmenu', (e) => {
          e.preventDefault();
          const item = getItemFromRow();
          if (item) this.openTableActionMenu(codeCell, item);
        });

        // [Chuẩn Mobile] Chạm giữ (Long Press) mở thẻ Quick Action
        let pressTimer;
        codeCell.addEventListener('touchstart', (e) => {
          pressTimer = window.setTimeout(() => {
            const item = getItemFromRow();
            if (item) this.openTableActionMenu(codeCell, item);
          }, 600); // Đè 600ms
        });
        codeCell.addEventListener('touchend', () => { clearTimeout(pressTimer); });
        codeCell.addEventListener('touchmove', () => { clearTimeout(pressTimer); });
      }
    });
...
`;
  ...
  // ============== MENU ACTION ================
  openTableActionMenu(anchorEl, item) {
    ...
    menu.style.left = Math.max(10, Math.min(rect.left + rect.width / 2, window.innerWidth - 210)) + 'px';
    ...
    menu.innerHTML = `
      ...
      <button class="tam-act" data-act="detail"
      ...
      <button class="tam-act" data-act="checkin"
      ...
      <button class="tam-act" data-act="checkout"
          style="width:100%;padding:10px;border-radius:10px;border:1px solid rgba(0,0,0,0.10);background:#fff;cursor:pointer;font-weight:700;">
          出庫 / Check-out
      </button>
      ...
      <button class="tam-act" data-act="inventory"
          style="width:100%;padding:10px;border-radius:10px;border:1px solid rgba(0,0,0,0.10);background:#fff;cursor:pointer;font-weight:700;">
          棚卸 / Kiểm kê
      </button>
      ...
      <button class="tam-act" data-act="qr"
          style="width:100%;padding:10px;border-radius:10px;border:1px solid rgba(0,0,0,0.10);background:#fff;cursor:pointer;font-weight:700;">
          QRコード / Mã QR
      </button>
      ...
      <button class="tam-act" data-act="move"
          style="width:100%;padding:10px;border-radius:10px;border:1px solid rgba(0,0,0,0.10);background:#fff;cursor:pointer;font-weight:700;">
          位置 / Location
      </button>
      ...
      <button class="tam-act" data-act="open-storage-module"
      ...
      <button class="tam-act" data-act="close"
